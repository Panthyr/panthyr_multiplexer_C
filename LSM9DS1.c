...
uint16_t LSM9DS1_init(imu_t* imu, const imu_config_t* config)
{
	...
	imu->settings.device.mAddress = IMU_M_ADDR;
	...
	imu->settings.mag.XYPerformance = config->low_power_mode ? 0 : 3; 
        // if set to low power mode, set to 0, otherwise set to 3
...
void LSM9DS1_constrainScales(imu_t* imu)
{
	if ((imu->settings.gyro.scale != 245) && (imu->settings.gyro.scale != 500) &&
		(imu->settings.gyro.scale != 2000))
            // options are 245, 500 and 2000. If not one of these, set to 245
	{
		imu->settings.gyro.scale = 245;
	}
	...
	if ((imu->settings.accel.scale != 2) && (imu->settings.accel.scale != 4) &&
		(imu->settings.accel.scale != 8) && (imu->settings.accel.scale != 16))
            // options are 2, 4, 8 and 16. If not one of these, set to 2
	{
		imu->settings.accel.scale = 2;
	}
	...
	if ((imu->settings.mag.scale != 4) && (imu->settings.mag.scale != 8) &&
		(imu->settings.mag.scale != 12) && (imu->settings.mag.scale != 16))
            // options are 4, 8, 12 and 16. If not one of these, set to 4
	{
		imu->settings.mag.scale = 4;
	}
}
...
{
	LSM9DS1_I2CwriteByte(imu, imu->settings.device.agAddress, subAddress, data);
}

void LSM9DS1_mWriteByte(imu_t* imu, uint8_t subAddress, uint8_t data)
{
	return LSM9DS1_I2CwriteByte(imu, imu->settings.device.mAddress, subAddress, data);
}

uint8_t LSM9DS1_xgReadByte(imu_t* imu, uint8_t subAddress)
{
	return LSM9DS1_I2CreadByte(imu, imu->settings.device.agAddress, subAddress);
}

void LSM9DS1_xgReadBytes(imu_t* imu, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	LSM9DS1_I2CreadBytes(imu, imu->settings.device.agAddress, subAddress, dest, count);
}

uint8_t LSM9DS1_mReadByte(imu_t* imu, uint8_t subAddress)
{
	return LSM9DS1_I2CreadByte(imu, imu->settings.device.mAddress, subAddress);
}

void LSM9DS1_mReadBytes(imu_t* imu, uint8_t subAddress, uint8_t * dest, uint8_t count)
{
	LSM9DS1_I2CreadBytes(imu, imu->settings.device.mAddress, subAddress, dest, count);
}

void LSM9DS1_I2CwriteByte(imu_t * imu, uint8_t address, uint8_t subAddress, uint8_t data)
{

    I2C1_Write(address, subAddress, &data, 1);
}
...
uint8_t LSM9DS1_I2CreadByte(imu_t * imu, uint8_t address, uint8_t subAddress)
{
	...
	// Value is returned to `temp`.
        I2C1_Read(address, subAddress, &retval, 1);
	return retval;
}
...
{
	// gyro/Acc returns bytes starting at register address as long as master replyies with ACK
        // mag needs the MSO of the address set to 1 to autoincrement the read registers
    
    if ((count > 1) && (address == imu->settings.device.mAddress)) {
        subAddress |= 0b10000000;
    }
    
    I2C1_Read(address, subAddress, dest, count);
}
